# -*- coding: utf-8 -*-
"""Thesis_Code_Zecchin_4_25.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1_gvYzkhrsYOUloOyFhSRF8whbLparjrH
"""
...
coords = transform_coords(org_coords,sym_arr,trans_arr)

def molecule_details(atoms,atoms_list):
  total = 0
  for name in atoms_list:
    num = np.sum(atoms[:,0] == name)
    print(name,num)
    total+=num
...
easf_base = easf_base * sample_rate # length is 101
...
def phase_method():
  ...
  for i in np.arange(22300,len(atom_names)):
    name = atom_names[i]
...
def dirac_delta():
  ...
  for name in atoms_list:
    ...
    positions = coords[np.where(atom_names == name),0:3]
    indices = positions[0,:,1].shape[0]
    ...
    xsub[0:indices] = np.array(positions[0,:,0])
    ysub[0:indices] = np.array(positions[0,:,1])
    zsub[0:indices] = np.array(positions[0,:,2])
    x = (xsub/voxel_size*rescale)
...
with mrcfile.new('molecule.mrc',overwrite=True) as mrc:
...
"""# Power Spectrum by Atomic Breakdown"""
